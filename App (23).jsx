...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select("username, is_admin, avatar_url, banner_url, bio")
  ...
  const saveBio = async () => {
    if (!session) return;
    setBioSaving(true);
    const { error } = await supabase.from("profiles").update({ bio: bioDraft.trim() }).eq("id", session.user.id);
    setBioSaving(false);
    if (error) return setErrorMsg(error.message);
    setProfile((prev) => ({ ...prev, bio: bioDraft.trim() }));
  };
            ...
            return (
              <div style={{ background: "#170D0E", border: "1px solid #3D2226", borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: "0 24px 24px" }}>
                  <div style={{ ...styles.avatarWrap, width: 84, height: 84, marginTop: -20, border: "3px solid #170D0E", borderRadius: "50%", background: "#170D0E", position: "relative", zIndex: 2 }}>
                  ...
                  <div style={{ display: "flex", gap: 20, marginTop: 20, flexWrap: "wrap" }}>
                    <div style={{ flex: "1 1 180px", maxWidth: 220 }}>
                      <div style={{ ...styles.cardTitle, fontSize: 13.5, marginBottom: 10 }}>Друзья ({viewingUserFriends.length})</div>
                    ...
                    <div style={{ flex: "2 1 320px", minWidth: 0 }}>
                      <div style={{ ...styles.cardTitle, fontSize: 13.5, marginBottom: 8 }}>О себе</div>
                      <div style={{ fontSize: 13, color: viewingUser.bio ? "#E8DCDB" : "#8C7876", lineHeight: 1.5, marginBottom: 18, whiteSpace: "pre-wrap" }}>
