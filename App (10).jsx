...
export default function App() {
  ...
  const refreshFriends = useCallback(async (userId) => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, status, sender_id, receiver_id, sender:profiles!friend_requests_sender_id_fkey(id, username, avatar_url), receiver:profiles!friend_requests_receiver_id_fkey(id, username, avatar_url)"
      )
  ...
  const heroBeforeOp = heroProgress < 0.35 ? 1 - heroProgress / 0.35 : 0;
  let heroSmokeOp = 0;
  if (heroProgress < 0.3) heroSmokeOp = heroProgress / 0.3;
  else if (heroProgress < 0.55) heroSmokeOp = 1;
  else if (heroProgress < 0.85) heroSmokeOp = 1 - (heroProgress - 0.55) / 0.3;
  const heroAfterOp = heroProgress > 0.6 ? Math.min((heroProgress - 0.6) / 0.3, 1) : 0;
    ...
    <div style={styles.page}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @keyframes nur-drip-fall {
          0%   { opacity: 0; height: 9px; transform: translateY(0) scaleY(1); }
          10%  { opacity: 1; height: 10px; transform: translateY(0) scaleY(1.05); }
          28%  { height: 24px; transform: translateY(2px) scaleY(1.35); }
          40%  { height: 33px; transform: translateY(6px) scaleY(1.55); }
          52%  { height: 15px; transform: translateY(24px) scaleY(1.1); opacity: 1; }
          62%  { height: 12px; transform: translateY(36px) scaleY(1); opacity: 0.9; }
          78%  { height: 10px; transform: translateY(51px) scaleY(0.9); opacity: 0.5; }
          92%  { opacity: 0.08; transform: translateY(63px); }
          100% { opacity: 0; height: 9px; transform: translateY(0) scaleY(1); }
        }
        .nur-drip {
          position: absolute;
          top: 0;
          width: 7px;
          background: linear-gradient(#C4222E, #5A0F14);
          border-radius: 50% 50% 50% 50% / 65% 65% 35% 35%;
          animation-name: nur-drip-fall;
      ...
      <div style={styles.body}>
        ...
        {activeTab === "tournaments" && (
          <>
          <div ref={heroRef} style={styles.heroWrap}>
            <div style={{ ...styles.heroTitleLayer, opacity: heroBeforeOp }}>
              <div style={styles.heroTitleText}>NUR TOURNAMENTS</div>
              <div style={{ position: "relative", height: 70 }}>
                <div className="nur-drip" style={{ left: "30%", animationDuration: "5.2s", animationDelay: "0s" }} />
                <div className="nur-drip" style={{ left: "40%", animationDuration: "6.7s", animationDelay: "1.1s" }} />
                <div className="nur-drip" style={{ left: "52%", animationDuration: "5.9s", animationDelay: "2.3s" }} />
                <div className="nur-drip" style={{ left: "63%", animationDuration: "7.3s", animationDelay: "0.6s" }} />
                <div className="nur-drip" style={{ left: "72%", animationDuration: "6.1s", animationDelay: "3.4s" }} />
              </div>
            </div>

            <div style={{ ...styles.heroTitleLayer, opacity: heroAfterOp }}>
              <div style={styles.heroTitleText}>
                <span style={{ color: "#F3ECEA" }}>NUR</span> <span style={{ color: "#C4222E" }}>TOURNAMENTS</span>
              </div>
            </div>

            <div style={{ ...styles.heroSmoke, opacity: heroSmokeOp }} />
          </div>
              ...
              <>
                ...
                <div style={styles.card}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {incomingRequests.map((r) => (
                          <div key={r.requestId} style={styles.friendRow}>
                            ...
                            <span style={{ flex: 1, fontSize: 13.5 }}>{r.username}</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
                    ...
                    {friends.map((f) => {
                      const friendTeams = teams.filter((t) => (t.team_members || []).some((m) => m.member_name === f.username));
                      const isOpen = expandedFriendId === f.requestId;
                      return (
                        <div key={f.requestId}>
                          <div style={{ ...styles.friendRow, cursor: "pointer" }} onClick={() => setExpandedFriendId(isOpen ? null : f.requestId)}>
                            <div style={styles.avatarWrapSm}>
                              {f.avatar_url ? (
                                <img src={f.avatar_url} alt="" style={styles.avatarImgSm} />
                              ) : (
                            ...
                            <span style={{ flex: 1, fontSize: 13.5 }}>{f.username}</span>
                            <button
                          ...
                          {isOpen && (
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 6, paddingLeft: 12 }}>
                              {friendTeams.length === 0 ? (
                                <span style={{ ...styles.hint, fontSize: 11.5 }}>Не состоит ни в одной команде.</span>
                              ) : (
                                friendTeams.map((t) => (
                      ...
                      <div key={ad.id} style={{ border: "1px solid #3D2226", borderRadius: 8, padding: 12 }}>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                          <input
                            ...
                            accept="image/*"
                            id={`ad-file-${ad.slot}`}
                            style={{ color: "#A08A8C", fontSize: 12, flex: "1 1 160px" }}
                          />
                          <input
                            className="nur-in"
...
const styles = {
  ...
  adSlotFilled: { display: "block", border: "1px solid #3D2226", borderRadius: 8, overflow: "hidden" },
  adSlotImg: { width: "100%", display: "block" },
  adSlotEmpty: {
    display: "flex",
    flexDirection: "column",
    alignItems: "center",
  ...
  heroWrap: {
    ...
    borderRadius: 10,
    overflow: "hidden",
    background: "linear-gradient(#0A0708,#170D0E)",
    marginBottom: 20,
  ...
  heroTitleText: {
    ...
    letterSpacing: 4,
    color: "#C4222E",
    textShadow: "0 0 24px rgba(196,34,46,0.55)",
    textAlign: "center",
  },
  heroSmoke: {
    position: "absolute",
    ...
    filter: "blur(26px)",
    background:
      "radial-gradient(ellipse 60% 60% at 30% 55%, rgba(10,7,8,0.98), transparent 62%), radial-gradient(ellipse 65% 60% at 70% 50%, rgba(10,7,8,0.98), transparent 64%), radial-gradient(ellipse 80% 65% at 50% 80%, rgba(10,7,8,0.95), transparent 62%), radial-gradient(ellipse 50% 50% at 50% 20%, rgba(217,65,76,0.25), transparent 60%)",
  },
};
