...
export default function App() {
  ...
  }, [session, refreshFriends, refreshTeamInvites, refreshSupportTickets, refreshSupportArchive, refreshReadyChecks, refreshMatchVetoes, refreshMapImages]);
  ...
  }, [refreshTeams, refreshTournaments, refreshAllMatches, refreshReadyChecks, refreshMatchVetoes, refreshMapImages, refreshAds, refreshStaffProfiles]);
    ...
    <div style={styles.page}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .nur-veto-card { transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
        .nur-veto-card.pickable:hover {
          transform: translateY(-6px);
          border-color: #D9414C !important;
          box-shadow: 0 14px 34px rgba(0,0,0,0.55), 0 0 26px rgba(217,65,76,0.4);
        }
              ...
              <>
                ...
                <div style={styles.card}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    {MAP_POOL.map((mapKey) => (
                      <div key={mapKey} style={{ border: "1px solid #3D2226", borderRadius: 8, padding: 12 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <span style={{ color: "#F3ECEA", fontSize: 13, fontWeight: 600 }}>{MAP_LABEL[mapKey]}</span>
                          {mapImages[mapKey] && (
                            <button style={{ ...styles.ghostBtnSm, borderColor: "#FF5A5A", color: "#FF5A5A" }} onClick={() => removeMapImage(mapKey)}>
                              Сбросить на заливку
                            </button>
                          )}
                        </div>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
                          <div style={{ flex: "1 1 160px" }}>
                            <FileChooser id={`map-file-${mapKey}`} />
                          </div>
                          <button
                            className="nur-btn"
                            style={styles.accentBtnSm}
                          ...
                          >
                            {mapImageUploading[mapKey] ? "Сохраняем…" : "Сохранить"}
                          </button>
                        </div>
                      </div>
                    ))}
      ...
      {(() => {
        ...
        return (
          <div style={styles.readyCheckOverlay}>
            <div style={styles.vetoBgWall}>
              {[0, 1, 2, 3, 4].map((row) => (
                <div key={row} style={styles.vetoBgRow}>
                  {[0, 1, 2, 3, 4, 5].map((i) => (
                    <span key={i}>
                      <span style={styles.vetoBgNur}>NUR</span>
                      <span style={styles.vetoBgTour}>TOURNAMENTS</span>
                    </span>
                  ))}
                </div>
              ))}
            </div>
            <div style={{ ...styles.readyCheckCard, maxWidth: 900, background: "transparent", border: "none", boxShadow: "none", padding: 0, position: "relative" }}>
              {profile?.is_admin && (
                <button
              ...
              <div style={{ fontSize: 18, fontWeight: 800, color: "#F3ECEA", marginBottom: 4, textAlign: "center", position: "relative", zIndex: 1 }}>
                {myTeam ? teamLabel(myTeam) : "Вы"} <span style={{ color: "#D9414C" }}>VS</span> {oppTeam ? teamLabel(oppTeam) : "Соперник"}
              </div>
              {myVeto.status === "completed" ? (
                <div style={{ ...styles.hint, marginBottom: 20, textAlign: "center", position: "relative", zIndex: 1 }}>Карта определена</div>
              ) : (
                <div style={{ ...styles.hint, marginBottom: 24, textAlign: "center", color: "#E8A33D", position: "relative", zIndex: 1 }}>
                  {!isCaptain
                    ? `Карту банит капитан. Сейчас ход: ${turnTeam ? teamLabel(turnTeam) : "…"}`
                    : myTurn
                    ? "Ваш ход — забаньте карту"
                    : `Сейчас банит: ${turnTeam ? teamLabel(turnTeam) : "…"}`}
                </div>
              )}
              <div style={{ display: "flex", gap: 10, overflowX: "auto", padding: "4px 2px 10px", position: "relative", zIndex: 1 }}>
                {MAP_POOL.map((mapKey) => {
                  ...
                  return (
                    <div
                      key={mapKey}
                      className={clickable ? "nur-veto-card pickable" : "nur-veto-card"}
                      onClick={() => clickable && banMap(myVeto, mapKey)}
                      style={{
                        ...styles.vetoCard,
                        background: mapImages[mapKey] ? `url(${mapImages[mapKey]}) center/cover` : MAP_GRADIENT[mapKey],
                        cursor: clickable ? "pointer" : "default",
                        filter: banned ? "grayscale(0.85) brightness(0.4)" : "none",
                        borderColor: isDecider ? "#E8A33D" : clickable ? "#4A2C2F" : "#3D2226",
                        boxShadow: isDecider ? "0 0 34px rgba(232,163,61,0.5), 0 14px 40px rgba(0,0,0,0.6)" : "none",
                        transform: isDecider ? "translateY(-10px)" : "none",
                      }}
                    >
                      {banTeam && (
                        <div style={{ ...styles.vetoTeamBadge, borderColor: isGoldBadge ? "#E8A33D" : "#D9414C", boxShadow: isGoldBadge ? "0 0 18px rgba(232,163,61,0.45)" : "0 0 18px rgba(217,65,76,0.45)" }}>
                          {(banTeam.tag || teamLabel(banTeam)).slice(0, 4).toUpperCase()}
                        </div>
                      )}
                      <div style={{ position: "relative", padding: "10px 6px 14px", textAlign: "center" }}>
                        <div
                          style={{
                            ...
                            ...(banned
                              ? { color: "#6b5a58" }
                              : { background: isDecider ? "linear-gradient(90deg,#fff,#E8A33D)" : "linear-gradient(90deg,#E8A33D,#D9414C)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }),
                          }}
                        >
...
const styles = {
  ...
  vetoBgNur: { color: "rgba(255,255,255,0.05)" },
  vetoBgTour: { color: "transparent", WebkitTextStroke: "1.5px rgba(255,255,255,0.055)" },
  vetoCard: {
    position: "relative",
    flex: "0 0 108px",
    height: 300,
