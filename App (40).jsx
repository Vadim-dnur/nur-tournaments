...
function ModeToggle({ value, onChange }) {
  const [puff, setPuff] = useState(null); // { mode, id }
  ...
  const handleClick = (m) => {
    ...
    handleClick._t = window.setTimeout(() => {
      setPuff((prev) => (prev && prev.id === id ? null : prev));
    }, 900);
  };
  ...
  return (
    <div style={{ display: "flex", gap: 10 }}>
      {["5x5", "2x2"].map((m) => (
        <div key={m} style={{ position: "relative", flex: 1, overflow: "visible" }}>
          <button
            className={`nur-mode-btn${value === m ? " active" : ""}`}
            onClick={() => handleClick(m)}
            style={{ ...styles.modeBtn, ...(value === m ? styles.modeBtnActive : {}), width: "100%", textAlign: "center" }}
          >
            {MODE_LABEL[m]}
          </button>
          {puff && puff.mode === m && (
            <div key={puff.id} className="nur-mode-puff" aria-hidden="true">
              <span className="nur-mode-puff-a" />
              <span className="nur-mode-puff-b" />
              <span className="nur-mode-puff-c" />
              <span className="nur-mode-puff-d" />
            </div>
          )}
        </div>
      ))}
...
export default function App() {
  ...
  const refreshSupportTickets = useCallback(async () => {
    const { data, error } = await supabase.from("support_messages").select("user_id, created_at").order("created_at", { ascending: false });
    if (error) return;
    const seen = new Map();
    (data || []).forEach((m) => {
      if (!seen.has(m.user_id)) seen.set(m.user_id, m.created_at);
    });
    const ids = [...seen.keys()];
    if (!ids.length) return setSupportTickets([]);
    const { data: profs } = await supabase.from("profiles").select("id, username, avatar_url").in("id", ids);
    const list = ids
      .map((id) => {
        const p = (profs || []).find((pr) => pr.id === id);
        return p ? { user_id: id, username: p.username, avatar_url: p.avatar_url, last_at: seen.get(id) } : null;
      })
      .filter(Boolean);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages" }, (payload) => {
        ...
        const viewingChatFor = supportPanelOpenRef.current && supportViewRef.current === "chat" ? (supportTargetRef.current ? supportTargetRef.current.id : session.user.id) : null;
        if (viewingChatFor && msg.user_id === viewingChatFor) {
  ...
  }, [session, refreshFriends, refreshTeamInvites, refreshSupportTickets]);
  ...
  }, [refreshTeams, refreshTournaments, refreshAllMatches, refreshAds, refreshStaffProfiles]);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (data?.is_admin || data?.is_moderator) refreshSupportTickets();
      });
    refreshFriends(session.user.id);
    refreshTeamInvites(session.user.id);
  }, [session, refreshFriends, refreshTeamInvites, refreshSupportTickets]);
  ...
  const sendSupportMessage = async () => {
    ...
    const { data, error } = await supabase
      .from("support_messages")
      .insert({ user_id: targetUserId, sender_id: session.user.id, content: text })
      .select()
      .single();
    if (error) return setErrorMsg(error.message);
    ...
    <div style={styles.page}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .nur-mode-btn { transition: background .25s ease, color .25s ease, border-color .25s ease, box-shadow .25s ease, transform .12s ease; }
        ...
        .nur-mode-puff { position: absolute; inset: 0; pointer-events: none; z-index: 6; overflow: visible; }
        .nur-mode-puff span {
          position: absolute;
          bottom: 30%;
          border-radius: 50%;
          filter: blur(9px);
          opacity: 0;
          background: radial-gradient(circle, rgba(255,224,170,0.95), rgba(232,163,61,0.65) 45%, rgba(217,65,76,0.35) 70%, transparent 80%);
          animation: nur-mode-puff-rise 0.85s ease-out forwards;
        }
        @keyframes nur-mode-puff-rise {
          0% { opacity: 0; transform: translate(var(--px, 0), 0) scale(0.3); }
          18% { opacity: 0.9; }
          100% { opacity: 0; transform: translate(var(--px, 0), -42px) scale(1.7); }
        }
        .nur-mode-puff-a { left: 14%; width: 20px; height: 20px; --px: -6px; animation-delay: 0s; }
        .nur-mode-puff-b { left: 38%; width: 28px; height: 28px; --px: 4px; animation-delay: .06s; }
        .nur-mode-puff-c { left: 62%; width: 24px; height: 24px; --px: -3px; animation-delay: .12s; }
        .nur-mode-puff-d { left: 82%; width: 18px; height: 18px; --px: 7px; animation-delay: .18s; }
          ...
          <div style={{ ...styles.stack, flex: "1 1 480px", minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              {[
                ...
                <div key={mode} style={{ ...styles.card, flex: "1 1 220px", minWidth: 220 }}>
                  <div style={styles.cardHeadRow}>
                    <div style={styles.cardTitle}>Топ · {MODE_LABEL[mode]}</div>
                    <Trophy size={15} color="#D9414C" />
                  </div>
                  {board.length === 0 ? (
                    <div style={{ ...styles.hint, marginTop: 10 }}>Пока нет завершённых матчей.</div>
                  ) : (
            ...
            {session && (profile?.is_admin || profile?.is_moderator) && (
              <>
                ...
                <div style={styles.card}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
                    {supportTickets.length === 0 && <span style={styles.hint}>Пока никто не писал в поддержку.</span>}
                    {supportTickets.map((t) => (
                      <div key={t.user_id} style={{ ...styles.friendRow, cursor: "pointer" }} onClick={() => openSupportChat(t)}>
                        <div style={{ position: "relative" }}>
                          <div style={styles.avatarWrapSm}>
                            {t.avatar_url ? (
                        ...
                        <span style={{ flex: 1, fontSize: 13.5 }}>{t.username}</span>
                        <span style={{ ...styles.hint, fontSize: 11 }}>{formatDateTime(t.last_at)}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {session && !supportPanelOpen && (
        <button
          ...
          onClick={() => ((profile?.is_admin || profile?.is_moderator) ? openSupportList() : openSupportChat(null))}
        >
      ...
      {supportPanelOpen && (
        <div style={{ ...styles.supportPanel, transform: `translate(${supportDrag.x}px, ${supportDrag.y}px)` }}>
          <div style={{ ...styles.chatHeader, cursor: "grab", userSelect: "none" }} onMouseDown={onSupportHeaderMouseDown}>
            {supportView === "list" ? (
              <div style={{ flex: 1, minWidth: 0, color: "#F3ECEA", fontSize: 13, fontWeight: 600 }}>Обращения в поддержку</div>
            ) : (
              <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 8 }}>
                {(profile?.is_admin || profile?.is_moderator) && (
                  <button style={styles.iconBtn} onMouseDown={(e) => e.stopPropagation()} onClick={openSupportList}>
                    <ChevronDown size={14} color="#AE9B99" style={{ transform: "rotate(90deg)" }} />
                  </button>
                )}
                {supportTarget ? (
                  <>
                    <span style={{ color: "#F3ECEA", fontSize: 13.5, fontWeight: 600 }}>{supportTarget.username}</span>
                    <button
                      style={{ ...styles.ghostBtnSm, padding: "3px 8px", fontSize: 11 }}
                      onMouseDown={(e) => e.stopPropagation()}
                      onClick={() => {
                        setSupportPanelOpen(false);
                        openUserProfile(supportTarget);
                      }}
                    >
                      Профиль
                    </button>
          ...
          {supportView === "list" ? (
            <div className="nur-chat-scroll" style={{ ...styles.chatMessages, gap: 8 }}>
              {supportTickets.length === 0 && (
                <div style={{ ...styles.hint, textAlign: "center", marginTop: 20 }}>Пока никто не написал в поддержку.</div>
              )}
              {supportTickets.map((t) => (
                <div key={t.user_id} style={{ ...styles.friendRow, cursor: "pointer" }} onClick={() => openSupportChat(t)}>
                  <div style={{ position: "relative" }}>
                  ...
                  <span style={{ flex: 1, fontSize: 13.5 }}>{t.username}</span>
                  <span style={{ ...styles.hint, fontSize: 10.5 }}>{formatDateTime(t.last_at)}</span>
                </div>
              ))}
            </div>
          ) : (
...
const styles = {
  ...
  modeBtn: { background: "linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0.01))", border: "1px solid #3D2226", borderRadius: 10, color: "#AE9B99", fontSize: 12, fontFamily: "'JetBrains Mono', monospace", letterSpacing: 1, padding: "11px 14px", cursor: "pointer" },
