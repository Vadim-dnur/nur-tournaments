...
export default function App() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "matches" }, () => refreshAllMatches())
      .subscribe();
    return () => {
      supabase.removeChannel(channel);
    };
  }, [refreshAds, refreshTournaments, refreshTeams, refreshAllMatches]);
  ...
  // CS2 при подтверждении матча. На последних 5 секундах бип выше и
  // резче, для ощущения срочности. Звук синтезируется через Web Audio
  // (так же, как звук слот-машины в коинфлипе), отдельного аудиофайла
  // для этого нет. Своя собственная секундная проверка (не завязана на
  // readyCheckNow выше) — опрашивает чаще (раз в 250мс), чтобы не
  // проскочить нужную целую секунду.
  const beepAudioCtxRef = useRef(null);
  const lastBeepSecondRef = useRef(null);
  const playCountdownBeep = (urgent) => {
    try {
      if (!beepAudioCtxRef.current) {
        const Ctx = window.AudioContext || window.webkitAudioContext;
        beepAudioCtxRef.current = new Ctx();
      }
      const ctx = beepAudioCtxRef.current;
      if (ctx.state === "suspended") ctx.resume();
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = "square";
      osc.frequency.value = urgent ? 1050 : 780;
      ...
      // Web Audio недоступен/заблокирован браузером — просто без звука.
    }
  };
  useEffect(() => {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, position: "relative", zIndex: 1 }}>
          ...
          {!session ? (
            ...
            <div ref={navAreaRef} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ position: "relative" }}>
                <div ref={avatarPillRef} style={{ ...styles.avatarPill, cursor: "default" }} onMouseDown={(e) => e.preventDefault()}>
                  <div
                    style={{ ...styles.avatarWrapPill, cursor: "pointer" }}
                    onClick={(e) => {
                      e.stopPropagation();
                      setNavMenuOpen(false);
                      setNotifOpen(false);
      ...
      <div style={styles.body}>
        ...
        {activeTab === "tournaments" && (
          ...
          <div style={styles.hero}>
            ...
            <div style={styles.heroBadge}>● Приём заявок открыт</div>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 26, flexWrap: "wrap", position: "relative", zIndex: 1 }}>
              <button className="nur-btn" style={styles.accentBtn} onClick={() => setActiveTab("teams")}>
                Создать команду →
              </button>
              <a href="https://t.me/tourNUR" target="_blank" rel="noopener noreferrer" className="nur-btn" style={{ ...styles.ghostBtn, textDecoration: "none" }}>
                Telegram-канал
              </a>
            </div>
          </div>
          ...
          <div style={styles.statsRow}>
            ...
            <div style={styles.statCard}>
              <div style={styles.statNum}>{tournaments.filter((t) => t.status === "live").length}</div>
              ...
              {(() => {
                ...
                return board.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", marginTop: 14 }}>
                    {board.map((t, i) => (
                      <div key={t.id} style={styles.lbRow}>
                        <span style={{ ...styles.lbPos, ...(RANK_STYLE[i] || {}) }}>{i + 1}</span>
                        <span style={styles.lbName}>{t.name}</span>
                        <span style={styles.lbPts}>{t.titles > 0 ? `${t.titles} 🏆` : `${t.wins} W`}</span>
                      </div>
                    ))}
                  </div>
                );
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        {t.logo_url && (
                          <img
                            src={t.logo_url}
                            alt=""
                            style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", border: "1px solid #3D2226", flexShrink: 0 }}
                          />
                        )}
                ...
                {[...friends]
                  ...
                  .map((f) => (
                    ...
                    >
                      ...
                      <span style={{ flex: 1, fontSize: 13, color: "#F3ECEA" }}>{f.username}</span>
                      {unreadCounts[f.id] > 0 && <span style={styles.notifyDot} />}
                    </div>
                  ))}
              </div>
              ...
              {!activeChatFriend ? (
                ...
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: "1px solid #2A1B1D" }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 600, color: "#F3ECEA" }}>{activeChatFriend.username}</span>
                  </div>
          ...
          (() => {
            ...
            return (
              <div>
                ...
                <div style={{ flex: "0 0 240px", minWidth: 220, display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  {userTeams.length > 0 && (
                    <div style={{ background: "#170D0E", border: "1px solid #3D2226", borderRadius: 12, overflow: "hidden", marginTop: 16 }}>
                      <div style={{ padding: "14px 16px", borderBottom: "1px solid #2E1B1E", display: "flex", alignItems: "center", gap: 8 }}>
                        <ShieldPlus size={15} color="#E8A33D" />
                        <span style={{ color: "#E8A33D", fontWeight: 700, fontSize: 13 }}>КОМАНДА</span>
                      </div>
                      <div style={{ padding: 10 }}>
                        {userTeams.map((t, idx) => {
                          const expanded = expandedRosterTeamId === t.id;
                          return (
                            <div key={t.id} style={{ marginTop: idx > 0 ? 12 : 0 }}>
                              <div
                                style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}
                                onClick={() => setExpandedRosterTeamId(expanded ? null : t.id)}
                              >
                                {t.logo_url ? (
                                  <img src={t.logo_url} alt="" style={{ width: 32, height: 32, borderRadius: 7, objectFit: "cover", border: "1px solid #3D2226", flexShrink: 0 }} />
                                ) : (
                                  <div
                                    style={{
                                      width: 32,
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ fontSize: 13, color: "#F3ECEA", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                    {teamLabel(t)}
                                  </div>
                                  <div style={{ fontSize: 11, color: "#7A6668" }}>{MODE_LABEL[t.mode]}</div>
                                </div>
                                <ChevronDown size={13} color="#8C7876" style={{ transform: expanded ? "rotate(180deg)" : "none", transition: "transform 0.2s ease", flexShrink: 0 }} />
                              </div>
                              {expanded && (
                                <div style={{ marginTop: 8, marginLeft: 42, display: "flex", flexDirection: "column", gap: 6 }}>
                                  {(t.team_members || []).map((m) => (
                                    <div
                                      ...
                                      onClick={() => openTeamMemberProfile(m.member_name)}
                                    >
                                      <div style={styles.avatarWrapSm}>
                                        <div style={styles.avatarFallbackSm}>{(m.member_name || "?")[0].toUpperCase()}</div>
                                      </div>
                                      <span style={{ fontSize: 12.5, color: "#F3ECEA" }}>{m.member_name}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                ...
                <div style={{ flex: "1 1 420px", minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
                          ...
                          return (
                            <>
                              <div style={{ display: "flex", gap: 22, marginTop: 12, flexWrap: "wrap" }}>
                                <span style={{ fontSize: 12.5, color: "#AE9B99" }}>
                                  Место в топе: <b style={{ color: "#F3ECEA" }}>{bestRank ? `#${bestRank}` : "—"}</b>
                                </span>
                                <span style={{ fontSize: 12.5, color: "#AE9B99" }}>
                                  Титулов: <b style={{ color: "#F3ECEA" }}>{titleCount}</b>
                                </span>
                                <span style={{ fontSize: 12.5, color: "#AE9B99" }}>
                                  Побед: <b style={{ color: "#F3ECEA" }}>{winCount}</b>
                                </span>
                              </div>
                              <div style={{ display: "flex", gap: 32, marginTop: 14, paddingTop: 14, borderTop: "1px solid #241618" }}>
                                <div>
                                  <div style={{ fontSize: 10.5, color: "#7A6668", textTransform: "uppercase", letterSpacing: 0.5 }}>Команд</div>
                                  <div style={{ fontSize: 13.5, color: "#F3ECEA", marginTop: 3, fontWeight: 600 }}>{userTeams.length}</div>
                                </div>
                              </div>
                            </>
                          );
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 4, borderBottom: "1px solid #2E1B1E", padding: "0 4px" }}>
                      {isOwnProfile && (
                        <div
                          style={{ marginRight: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: "#6E5F5D", cursor: "pointer", padding: "10px 6px" }}
                          onClick={() => {
                            setActiveTab("profile");
                            setViewingUser(null);
                          }}
                      ...
                      {[
                        ...
                        { id: "matches", label: `Матчи (${userMatches.length})` },
                      ].map((tb) => (
                        <div
                          key={tb.id}
                          ...
                          style={{
                            ...
                            fontSize: 13,
                            cursor: "pointer",
                            color: viewingProfileTab === tb.id ? "#F3ECEA" : "#8C7876",
        ...
        <div style={styles.readyCheckOverlay}>
          <div style={{ ...styles.readyCheckCard, maxWidth: 560, maxHeight: "82vh", overflowY: "auto", textAlign: "left", position: "relative" }}>
            ...
            <ol style={{ margin: 0, paddingLeft: 20, display: "flex", flexDirection: "column", gap: 10, fontSize: 13, color: "#D8CBC9", lineHeight: 1.5 }}>
              ...
              <li>Администраторы могут не засчитать вам победу в случае выигрыша и временно отстранить от турниров до проверки записей матчей на читы.</li>
...
const styles = {
  ...
  heroTitleAccent: { color: "transparent", WebkitTextStroke: "1.6px rgba(255,255,255,0.55)" },
