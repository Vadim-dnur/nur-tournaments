...
export default function App() {
  ...
  const uploadTeamLogo = async (teamId, file) => {
    ...
    const { error: updErr } = await supabase.from("teams").update({ logo_url: logoUrl }).eq("id", teamId);
    if (updErr) return setErrorMsg(updErr.message);
    refreshTeams();
  };
      ...
      <div style={{ ...styles.nav, position: "relative" }} onCopy={(e) => e.preventDefault()}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, position: "relative", zIndex: 1 }}>
          ...
          <button
            ...
            style={{ ...styles.tabBtn, ...(activeTab === "lfg" ? styles.tabBtnActive : {}) }}
          >
            <Search size={14} /> Поиск тиммейтов
          </button>
        </div>
                      ...
                      <div key={p.id} style={{ ...styles.card, position: "relative", overflow: "hidden", paddingTop: 26 }}>
                        ...
                        <div style={{ display: "flex", gap: 12 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 11.5, color: "#6FBF73", marginTop: 4 }}>
                              {needPlayer
                                ? team
                                  ? `Свободно мест: ${freeSlots} / ${team.max_size}`
                                  : "—"
