    ...
    <div style={styles.page}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .nur-mode-btn { transition: background .25s ease, color .25s ease, border-color .25s ease, box-shadow .25s ease; }
        .nur-mode-btn:hover:not(.active) { background: rgba(232,163,61,0.09); border-color: #E8A33D; color: #E8A33D; }
                  ...
                  <div style={styles.card}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {["5x5", "2x2"].map((m) => (
                        <button
                          key={m}
                          className={`nur-mode-btn${createTeamMode === m ? " active" : ""}`}
                          onClick={() => setCreateTeamMode(m)}
                          style={{ ...styles.modeBtn, ...(createTeamMode === m ? styles.modeBtnActive : {}), flex: 1, textAlign: "center" }}
                        >
                          {MODE_LABEL[m]}
                        </button>
                      ))}
                ...
                <div style={styles.card}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                    {["5x5", "2x2"].map((m) => (
                      <button
                        key={m}
                        className={`nur-mode-btn${newTourMode === m ? " active" : ""}`}
                        onClick={() => setNewTourMode(m)}
                        style={{ ...styles.modeBtn, ...(newTourMode === m ? styles.modeBtnActive : {}), flex: 1, textAlign: "center" }}
                      >
                        {MODE_LABEL[m]}
                      </button>
                    ))}
...
const styles = {
  ...
  modeBtn: { background: "rgba(255,255,255,0.03)", border: "1px solid #3D2226", borderRadius: 8, color: "#AE9B99", fontSize: 12, fontFamily: "'JetBrains Mono', monospace", letterSpacing: 0.5, padding: "9px 12px", cursor: "pointer" },
  modeBtnActive: { color: "#1C1416", fontWeight: 700, borderColor: "transparent", boxShadow: "0 0 14px rgba(232,163,61,0.45)" },
