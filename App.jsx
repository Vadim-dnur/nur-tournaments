...
export default function App() {
  ...
  }, [session, refreshFriends, refreshTeamInvites, refreshSupportTickets, refreshSupportArchive, refreshReadyChecks, refreshMatchVetoes, refreshMatchCoinflips, refreshMatchLobbies, refreshMapImages]);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      if (matchCoinflips.length) refreshMatchCoinflips();
      if (matchLobbies.length) refreshMatchLobbies();
    }, 4000);
    return () => clearInterval(id);
  }, [matchVetoes.length, matchCoinflips.length, matchLobbies.length, refreshMatchVetoes, refreshMatchCoinflips, refreshMatchLobbies]);
        ...
        return (
          <div style={styles.readyCheckOverlay}>
            <div style={{ ...styles.readyCheckCard, position: "relative" }}>
              ...
              {myCoinflip.status === "completed" &&
                (() => {
                  const myLobby = matchLobbies.find((l) => l.match_id === myCoinflip.match_id);
                  if (!myLobby || myLobby.status === "pending" || myLobby.status === "creating") {
                    return <div style={{ ...styles.hint, marginTop: 24, textAlign: "center" }}>Готовим сервер, подождите немного…</div>;
                  }
                  if (myLobby.status === "failed") {
                    return (
                  ...
                  return (
                    <div style={{ marginTop: 26, width: "100%", maxWidth: 420 }}>
