...
export default function App() {
  ...
  const [faceitMatches, setFaceitMatches] = useState([]);
  // Связки "имя команды на FACEIT" -> "команда на сайте". Без них результаты
  // FACEIT-турниров не могут начислять очки, т.к. с FACEIT приходит только текст.
  const [faceitLinks, setFaceitLinks] = useState([]);
  // Выбранный боевой состав по турнирам: { [tournamentId]: [memberName, ...] }
  const [lineupPick, setLineupPick] = useState({});
  // Черновик ручного посева: { [tournamentId]: [teamId, ...] } в нужном порядке
  ...
  const [newLfgTelegram, setNewLfgTelegram] = useState("");
  useEffect(() => {
  ...
  // Платформа хостинга: "cybershoke" — обычный турнир на своём боте (сетка,
  // вето, лобби ведёт NUR); "faceit" — турнир реально создаётся и играется
  // на самом FACEIT, здесь только зеркалируются результаты. Переключается
  // ДО создания и после уже не меняется на сайте (в БД это просто отдельная
  // колонка, но менять платформу задним числом означало бы путать данные
  // из двух разных источников — проще пересоздать турнир при ошибке).
  const [newTourPlatform, setNewTourPlatform] = useState("cybershoke");
  const [newTourFaceitUrl, setNewTourFaceitUrl] = useState("");
  // Способ жеребьёвки: random — пары тасует система, manual — админ
  // расставляет порядок сам перед формированием сетки.
  const [newTourSeeding, setNewTourSeeding] = useState("random");
  ...
  const refreshTeams = useCallback(async () => {
    const { data, error } = await supabase
      .from("teams")
      .select("id, mode, name, tag, owner_id, max_size, logo_url, team_members(member_name, role)")
      .order("created_at");
  ...
  const refreshTournaments = useCallback(async () => {
    const { data, error } = await supabase
      .from("tournaments")
      .select(
        "id, mode, name, status, banner_url, prize_pool, max_teams, announce_at, reg_open_at, start_at, created_at, platform, faceit_tournament_id, faceit_url, seeding, tournament_teams(team_id, lineup, seed)"
      )
      .order("created_at");
    if (error) return setErrorMsg(error.message);
    setTournaments(data || []);
  ...
  // Результаты FACEIT-турниров синк-скрипт бота пишет сюда отдельно
  // (см. faceit-sync.js) — сайт их только читает, никогда не пишет сам.
  const refreshFaceitMatches = useCallback(async () => {
    const { data, error } = await supabase.from("faceit_matches").select("*").order("round");
    if (error) return;
    setFaceitMatches(data || []);
  }, []);

  const refreshFaceitLinks = useCallback(async () => {
    const { data, error } = await supabase.from("faceit_team_links").select("*");
    if (error) return;
    setFaceitLinks(data || []);
  }, []);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "faceit_matches" }, () => refreshFaceitMatches())
      .on("postgres_changes", { event: "*", schema: "public", table: "faceit_team_links" }, () => refreshFaceitLinks())
      .subscribe();
    return () => {
  ...
  const refreshLfgPosts = useCallback(async () => {
    const { data, error } = await supabase
      .from("teammate_posts")
      .select("id, author_id, kind, mode, team_id, description, telegram_username, created_at, author:profiles!teammate_posts_author_id_fkey(id, username, avatar_url)")
      .order("created_at", { ascending: false });
    if (error) return;
    setLfgPosts(data || []);
  }, []);
  ...
  const createLfgPost = async () => {
    ...
    // Telegram — отдельное явное поле контакта, а не часть свободного текста,
    // поэтому не проходит через containsBlockedLink (то правило про ссылки
    // в описании/имени, а не про этот конкретный, специально для этого
    // предназначенный ввод). Просто нормализуем формат: убираем "@" и
    // пробелы, если человек их вписал по привычке.
    const tgRaw = newLfgTelegram.trim().replace(/^@/, "");
    if (tgRaw && !/^[A-Za-z0-9_]{5,32}$/.test(tgRaw)) {
      return setErrorMsg("Telegram-юзернейм: только латиница, цифры и подчёркивание, от 5 до 32 символов.");
    }
  ...
  const refreshTeamInvites = useCallback(async (userId) => {
    ...
    const { data, error } = await supabase
      .from("team_invites")
      .select("id, status, role, team:teams!team_invites_team_id_fkey(id, name, tag, mode)")
  ...
  // Размер БОЕВОГО СОСТАВА на матч (сколько игроков выходит на карту).
  // Это НЕ вместимость ростера — ростер теперь неограниченный, а из него
  // капитан выбирает состав под конкретный турнир.
  const lineupSizeForMode = (mode) => (mode === "5x5" ? 5 : 2);
  const ROSTER_CAP = 50; // техническая верхняя граница, чтобы список не разрастался бесконечно
  ...
  const createTeam = async () => {
    ...
    const { error: mErr } = await supabase.from("team_members").insert({ team_id: team.id, member_name: currentUsername, role: "player" });
  ...
  const inviteToTeam = async (team, profile, role = "player") => {
    if ((team.team_members || []).length >= (team.max_size || ROSTER_CAP)) {
      setErrorMsg("Ростер команды переполнен.");
      return;
    }
    if (role === "coach" && (team.team_members || []).some((m) => m.role === "coach")) {
      setErrorMsg("В команде уже есть тренер — сначала снимите текущего.");
      return;
    }
    const { error } = await supabase.from("team_invites").insert({ team_id: team.id, invited_id: profile.id, role });
  ...
  const acceptTeamInvite = async (invite) => {
    ...
    const { error: mErr } = await supabase
      .from("team_members")
      .insert({ team_id: invite.team.id, member_name: currentUsername, role: invite.role || "player" });
    if (mErr) setErrorMsg(mErr.message);
    refreshTeamInvites(session.user.id);
    refreshTeams();
  };
  ...
  // Ссылка на FACEIT может быть вида .../championship/<id>/... или
  // .../tournament/<id>/... — вытаскиваем сам UUID, откуда бы он ни был,
  // а не завязываемся на конкретный формат URL (он у них не единственный,
  // см. обсуждение championship vs tournament).
  const parseFaceitTournamentId = (url) => {
  ...
  const createTournament = async () => {
    ...
    if (newTourPlatform === "faceit" && !parseFaceitTournamentId(newTourFaceitUrl)) {
      return setErrorMsg("Не удалось распознать ID турнира в ссылке FACEIT — проверь, что скопировала полный адрес турнира.");
    }
    ...
    const { error } = await supabase.from("tournaments").insert({
      ...
      seeding: newTourSeeding,
      platform: newTourPlatform,
      faceit_tournament_id: newTourPlatform === "faceit" ? parseFaceitTournamentId(newTourFaceitUrl) : null,
      faceit_url: newTourPlatform === "faceit" ? newTourFaceitUrl.trim() : null,
    });
    setTourCreating(false);
    if (error) return setErrorMsg(error.message);
    setNewTourName("");
    ...
    setNewTourRegOpenAt("");
    setNewTourStartAt("");
    setNewTourPlatform("cybershoke");
    setNewTourFaceitUrl("");
    setNewTourSeeding("random");
    refreshTournaments();
  };
  ...
  // Регистрация теперь всегда несёт боевой состав: ростер может быть
  // сколь угодно большим, но на турнир заявляется ровно столько игроков,
  // сколько требует режим (5 или 2). Тренер в состав не входит — он не
  // занимает игровой слот.
  const registerTeam = async (tournamentId, teamId, lineup) => {
    const tour = tournaments.find((t) => t.id === tournamentId);
    const need = lineupSizeForMode(tour?.mode || "5x5");
    if (!Array.isArray(lineup) || lineup.length !== need) {
      return setErrorMsg(`Выберите ровно ${need} ${need === 5 ? "игроков" : "игрока"} в боевой состав.`);
    }
    const { error } = await supabase
      .from("tournament_teams")
      .insert({ tournament_id: tournamentId, team_id: teamId, lineup });
    if (error) return setErrorMsg(error.message);
    setLineupPick((prev) => ({ ...prev, [tournamentId]: [] }));
    refreshTournaments();
  };

  // Переключение игрока в/из боевого состава на конкретный турнир.
  const toggleLineupMember = (tournamentId, memberName, maxSize) => {
    setLineupPick((prev) => {
      const cur = prev[tournamentId] || [];
      if (cur.includes(memberName)) {
        return { ...prev, [tournamentId]: cur.filter((n) => n !== memberName) };
      }
      if (cur.length >= maxSize) return prev; // молча игнорируем — лимит уже набран
      return { ...prev, [tournamentId]: [...cur, memberName] };
    });
  };
  ...
  // Смена способа жеребьёвки для УЖЕ созданного турнира (форма создания
  // задаёт seeding только при заведении нового турнира) — нужна для тех,
  // кто создал турнир до появления этого выбора, или просто передумал.
  // Имеет смысл только пока сетка ещё не сформирована.
  // Ручная привязка имени команды с FACEIT к реальной команде сайта.
  const setFaceitTeamLink = async (tournamentId, faceitName, teamId) => {
    const { error } = await supabase
      .from("faceit_team_links")
      .upsert(
        { tournament_id: tournamentId, faceit_team_name: faceitName, team_id: teamId || null, auto_matched: false },
        { onConflict: "tournament_id,faceit_team_name" }
      );
    if (error) return setErrorMsg(error.message);
    refreshFaceitLinks();
  };

  const setTourSeeding = async (tournamentId, seeding) => {
    const { error } = await supabase.from("tournaments").update({ seeding }).eq("id", tournamentId);
    if (error) return setErrorMsg(error.message);
    refreshTournaments();
  };
  ...
  const generateBracket = async (tournamentId) => {
    ...
    // При ручной жеребьёвке порядок берём тот, что админ выставил в
    // seedOrder (или сохранённый seed в БД), а не тасуем случайно.
    let ordered;
    if (tour.seeding === "manual") {
      const manual = seedOrder[tournamentId];
      if (manual && manual.length === teamIds.length) {
        ordered = manual.slice();
      } else {
        ordered = (tour.tournament_teams || [])
          .slice()
          .sort((a, b) => (a.seed ?? 9999) - (b.seed ?? 9999))
          .map((tt) => tt.team_id);
      }
    } else {
      ordered = shuffleArr(teamIds);
    }
  ...
  const buildLadder = (mode) => {
    ...
    // Результаты FACEIT-турниров. Они лежат в отдельной таблице и содержат
    // только текстовые имена команд, поэтому засчитываются лишь тем командам,
    // для которых есть подтверждённая связка в faceit_team_links.
    // Несвязанные имена просто игнорируются — админ увидит их в панели
    // сопоставления и свяжет вручную.
    const linkFor = (tournamentId, name) => {
      const l = faceitLinks.find(
        (x) => x.tournament_id === tournamentId && x.faceit_team_name === name && x.team_id
      );
      return l ? l.team_id : null;
    };

    const faceitRel = faceitMatches.filter(
      (m) => tourIds.has(m.tournament_id) && m.status === "finished" && m.winner_name
    );
    const faceitMaxRound = {};
    faceitRel.forEach((m) => {
      if (m.round != null) {
        faceitMaxRound[m.tournament_id] = Math.max(faceitMaxRound[m.tournament_id] ?? 0, m.round);
      }
    });
    faceitRel.forEach((m) => {
      const winnerName = m.winner_name;
      const loserName = winnerName === m.team1_name ? m.team2_name : m.team1_name;
      const winId = linkFor(m.tournament_id, winnerName);
      const loseId = linkFor(m.tournament_id, loserName);
      if (winId) touch(winId).wins += 1;
      if (loseId) touch(loseId).losses += 1;
      const tour = tourById[m.tournament_id];
      if (
        winId &&
        tour &&
        ...
        m.round != null &&
        m.round === faceitMaxRound[m.tournament_id]
      ) {
        touch(winId).titles += 1;
      }
    });
  ...
  // Короткое имя для СТРОКИ (не для объекта команды) — та же логика
  // усечения, что и в shortTeamName, но для случаев, когда имени команды
  // взять неоткуда, кроме как готовой строкой (FACEIT-матчи хранят имена
  // команд текстом, у них нет записи в teams этого сайта вообще).
  const shortNameStr = (name) => {
    const n = name || "—";
    return n.length > 14 ? n.slice(0, 13) + "…" : n;
  };

  // Лента последних результатов для бегущей строки и карточек — объединяет
  // завершённые матчи ОБЕИХ платформ (CyberShoke ведёт их в matches, FACEIT
  // зеркалируется в faceit_matches синк-скриптом) в единый нормализованный
  // список, чтобы дальше в разметке не пришлось разбирать два разных
  // формата данных отдельно.
  const cybershokeResults = playedMatches
    .filter((m) => m.team1_score != null && m.team2_score != null)
    .map((m) => ({
      id: `cs-${m.id}`,
      team1Name: shortTeamName(teamMap[m.team1_id]),
      team2Name: shortTeamName(teamMap[m.team2_id]),
      team1Score: m.team1_score,
      team2Score: m.team2_score,
      t1Won: m.winner_id === m.team1_id,
      tourName: tourById[m.tournament_id]?.name || "",
      roundLabel: roundLabelShort(m.round, (buildRounds(m.tournament_id) || []).length || m.round + 1),
      sortKey: (tourOrderIdx[m.tournament_id] ?? 0) * 100000 + m.round * 1000 + m.match_index,
    }));

  const faceitResults = faceitMatches
    .filter((m) => m.team1_score != null && m.team2_score != null)
    .map((m) => ({
      id: `fc-${m.id}`,
      team1Name: shortNameStr(m.team1_name),
      team2Name: shortNameStr(m.team2_name),
      team1Score: m.team1_score,
      team2Score: m.team2_score,
      t1Won: m.winner_name != null && m.winner_name === m.team1_name,
      tourName: tourById[m.tournament_id]?.name || "",
      roundLabel: m.round != null ? `РАУНД ${m.round + 1}` : "FACEIT",
      sortKey: (tourOrderIdx[m.tournament_id] ?? 0) * 100000 + (m.round ?? 0) * 1000,
    }));

  const recentResults = cybershokeResults
    .concat(faceitResults)
        ...
        {(activeTab === "tournaments" || activeTab === "tourlist") && (
                ...
                {recentResults.slice(0, Math.max(2, 4 - liveLobbies.length)).map((m) => (
                  <div key={m.id} style={styles.homeMatchCard}>
                    <div style={styles.matchCardTop}>
                      ...
                      <span style={styles.matchMap}>{m.tourName}</span>
                    </div>
                    <div style={styles.matchRow}>
                      <div style={{ ...styles.matchSideL, borderLeft: m.t1Won ? "3px solid #E8A33D" : "3px solid transparent" }}>
                        <span style={{ ...styles.matchTeam, ...styles.unskew, color: m.t1Won ? "#F3ECEA" : "#8C7876" }}>{m.team1Name}</span>
                      </div>
                      <span style={styles.matchScore}>
                        <span style={{ color: m.t1Won ? "#E8A33D" : "#8C7876" }}>{m.team1Score}</span>
                        <span style={{ color: "#5A4548", margin: "0 3px" }}>:</span>
                        <span style={{ color: !m.t1Won ? "#E8A33D" : "#8C7876" }}>{m.team2Score}</span>
                      </span>
                      <div style={{ ...styles.matchSideR, borderRight: !m.t1Won ? "3px solid #E8A33D" : "3px solid transparent" }}>
                        <span style={{ ...styles.matchTeam, ...styles.unskew, textAlign: "right", color: !m.t1Won ? "#F3ECEA" : "#8C7876" }}>
                          {m.team2Name}
                        </span>
                      </div>
                    </div>
                    <div style={styles.matchFoot}>{m.roundLabel}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ ...styles.stack, flex: "1 1 480px", minWidth: 0 }}>
            ...
            {tourList.map((tour) => {
              ...
              return (
                <div key={tour.id} style={styles.tourCardOuter}>
                  {/* Шапка карточки в стиле трансляции: баннер (если есть) под
                      тёмной вуалью, статус-пилюля, формат и крупный заголовок. */}
                  <div
                    style={{
                      ...styles.tourHero,
                      ...(tour.banner_url
                        ? {
                            backgroundImage: `linear-gradient(180deg, rgba(10,7,8,0.55) 0%, rgba(10,7,8,0.92) 100%), url(${tour.banner_url})`,
                  ...
                  >
                    {!tour.banner_url && <span style={styles.tourGhost}>CUP</span>}
                    <div style={{ position: "relative", zIndex: 1 }}>
                      <div style={styles.tourHeroMetaRow}>
                        <span style={{ ...styles.tourStatusPill, ...(tour.status === "live" ? styles.tourStatusPillLive : {}) }}>
                          <span style={styles.unskew}>
                            {tour.status === "live" ? "● " : ""}
                            {STATUS_LABEL[tour.status]}
                          </span>
                        </span>
                        <span style={styles.tourHeroMeta}>
                          {MODE_LABEL[tour.mode]} · SINGLE ELIMINATION · ONLINE
                        </span>
                      </div>
                      <div
                        style={{ ...styles.tourHeroTitle, cursor: tour.status !== "registration" ? "pointer" : "default" }}
                  ...
                  {/* FACEIT-турниры не ведут внутреннюю сетку на этом сайте —
                      реальная сетка и матчи живут на FACEIT, здесь только
                      зеркалируются результаты, которые тянет faceit-sync.js. */}
                  {tour.platform === "faceit" ? (
                    <div style={{ padding: "16px 18px", borderTop: "1px solid #2A1A1C" }}>
                      <div style={styles.tourStatStrip}>
                        <div style={{ ...styles.tourStatCell, borderRight: "none" }}>
                          <div style={styles.tourStatLabel}>ХОСТИНГ</div>
                          <div style={{ ...styles.tourStatVal, color: "#E8A33D" }}>FACEIT</div>
                        </div>
                      </div>
                      {/* Панель сопоставления команд. Видна только админам.
                          Показывает все имена команд, встреченные в матчах
                          этого турнира, и позволяет привязать каждое к реальной
                          команде сайта — иначе очки за победы не начислятся. */}
                      {(profile?.is_admin || profile?.is_moderator) && (() => {
                        const names = Array.from(
                          new Set(
                            faceitMatches
                              .filter((m) => m.tournament_id === tour.id)
                              .flatMap((m) => [m.team1_name, m.team2_name])
                              .filter(Boolean)
                          )
                        );
                        if (!names.length) return null;
                        const linkOf = (n) =>
                          faceitLinks.find((x) => x.tournament_id === tour.id && x.faceit_team_name === n);
                        const unlinked = names.filter((n) => !linkOf(n)?.team_id).length;
                        const modeTeams = teams.filter((t) => t.mode === tour.mode);
                        return (
                          <div style={{ border: "1px solid #3D2226", padding: 12, marginTop: 14 }}>
                            <div style={{ ...styles.cardTitle, fontSize: 13 }}>
                              Команды: связь с сайтом{" "}
                              {unlinked > 0 ? (
                                <span style={{ color: "#E06C6C", fontSize: 12 }}>· не связано: {unlinked}</span>
                              ) : (
                                <span style={{ color: "#8BC34A", fontSize: 12 }}>· всё связано</span>
                              )}
                            </div>
                            <div style={{ ...styles.hint, marginBottom: 8 }}>
                              Очки за победы начисляются только связанным командам.
                            </div>
                            {names.map((n) => {
                              const link = linkOf(n);
                              return (
                                ...
                                >
                                  <span style={{ fontSize: 13, minWidth: 120 }}>{n}</span>
                                  <span style={{ color: "#5A4548" }}>→</span>
                                  <select
                                    value={link?.team_id || ""}
                                    onChange={(e) => setFaceitTeamLink(tour.id, n, e.target.value)}
                                    style={styles.select}
                                  >
                                  ...
                                  {link?.auto_matched && link?.team_id && (
                                    <span style={{ ...styles.hint, color: "#8BC34A" }}>авто</span>
                                  )}
                                </div>
                              );
                            })}
                      ...
                      {(() => {
                        const rows = faceitMatches.filter((m) => m.tournament_id === tour.id);
                        if (!rows.length) {
                          return (
                            <div style={{ ...styles.hint, marginTop: 12 }}>
                              Результаты появятся здесь, как только матчи начнутся на FACEIT (обновляется раз в несколько
                              минут).
                            </div>
                          );
                        }
                            ...
                            {rows.map((m) => (
                              <div key={m.id} style={styles.matchListRow}>
                                <div style={styles.matchListTop}>
                                  <span style={m.status === "finished" ? styles.doneTag : styles.liveTag}>
                                    {m.status === "finished" ? "ЗАВЕРШЁН" : m.status === "ongoing" ? "● LIVE" : "ПРЕДСТОИТ"}
                                  </span>
                                  {m.round != null && <span style={styles.matchMap}>РАУНД {m.round + 1}</span>}
                                </div>
                                <div style={styles.matchListBody}>
                                  <div style={styles.matchListSide}>
                                    <span
                                      style={{
                                        ...
                                        color: m.winner_name && m.winner_name !== m.team1_name ? "#7A6668" : "#F3ECEA",
                                      }}
                                    >
                                      {m.team1_name || "—"}
                                    </span>
                                  ...
                                  <div style={styles.matchListScore}>
                                    {m.team1_score != null && m.team2_score != null ? (
                                      <>
                                        <span style={{ color: m.winner_name === m.team1_name ? "#E8A33D" : "#7A6668" }}>{m.team1_score}</span>
                                        <span style={{ color: "#5A4548", margin: "0 4px" }}>:</span>
                                        <span style={{ color: m.winner_name === m.team2_name ? "#E8A33D" : "#7A6668" }}>{m.team2_score}</span>
                                      </>
                                    ) : (
                                  ...
                                  <div style={{ ...styles.matchListSide, justifyContent: "flex-end" }}>
                                    <span
                                      style={{
                                        ...styles.matchListName,
                                        textAlign: "right",
                                        color: m.winner_name && m.winner_name !== m.team2_name ? "#7A6668" : "#F3ECEA",
                                      }}
                                    >
                                      {m.team2_name || "—"}
                                    </span>
                      ...
                      {tour.faceit_url && (
                        <a
                          href={tour.faceit_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="nur-btn"
                          style={{ ...styles.heroBtnPrimary, marginTop: 14, display: "inline-flex", textDecoration: "none" }}
                        >
                          <span style={styles.unskew}>Открыть на FACEIT</span>
                        </a>
                      )}
                    </div>
                  ) : (
                    <>
                  {(() => {
                    const rr = buildRounds(tour.id);
                    const played = allBracketMatches.filter((m) => m.tournament_id === tour.id && m.winner_id).length;
                    const totalM = allBracketMatches.filter((m) => m.tournament_id === tour.id).length;
                    const stage = rr.length
                      ? (() => {
                          for (let k = 0; k < rr.length; k++) {
                            if (rr[k].some((m) => m && m.team1_id && m.team2_id && !m.winner_id)) return roundLabelShort(k, rr.length);
                          }
                          return roundLabelShort(rr.length - 1, rr.length);
                        })()
                      : "—";
                    return (
                      <div style={styles.tourStatStrip}>
                        ...
                        <div style={styles.tourStatCell}>
                          ...
                          <div style={styles.tourStatVal}>{totalM ? `${played} / ${totalM}` : "—"}</div>
                        </div>
                        <div style={{ ...styles.tourStatCell, borderRight: "none" }}>
                  ...
                  <div style={{ padding: 18 }}>
                    <div style={styles.tourActionRow}>
                      {tour.status === "bracket_ready" && (profile?.is_admin || profile?.is_moderator) && (
                        <button className="nur-btn" style={{ ...styles.heroBtnPrimary, background: "#E8A33D", color: "#2C1B06" }} onClick={() => startTournament(tour.id)}>
                          <span style={styles.unskew}>Начать турнир</span>
                        </button>
                      )}
                      {tour.status !== "registration" && (
                        <button className="nur-btn" style={styles.heroBtnPrimary} onClick={() => toggleExpand(tour.id, true)}>
                          <span style={styles.unskew}>Смотреть сетку</span>
                        </button>
                      )}
                      {tour.start_at && <span style={styles.tourStartMeta}>Старт: {formatDateTime(tour.start_at)}</span>}
                    </div>
                    ...
                    {tour.status === "registration" && (
                      ...
                      {session && !isFull && eligibleTeams.length > 0 && (() => {
                        const pickedTeamId = regSelections[tour.id] || eligibleTeams[0].id;
                        const pickedTeam = teamMap[pickedTeamId] || eligibleTeams[0];
                        const need = lineupSizeForMode(tour.mode);
                        const roster = (pickedTeam?.team_members || []).filter((m) => m.role !== "coach");
                        const coach = (pickedTeam?.team_members || []).find((m) => m.role === "coach");
                        const chosen = lineupPick[tour.id] || [];
                          ...
                          <div style={{ width: "100%" }}>
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                              <select
                                value={pickedTeamId}
                                onChange={(e) => {
                                  setRegSelections({ ...regSelections, [tour.id]: e.target.value });
                                  setLineupPick((prev) => ({ ...prev, [tour.id]: [] }));
                                }}
                                style={styles.select}
                              >
                                {eligibleTeams.map((t) => (
                              ...
                              <span style={{ ...styles.hint, color: enough ? "#8BC34A" : "#E8A33D" }}>
                                Состав: {chosen.length} / {need}
                              </span>
                            </div>

                            {roster.length < need ? (
                              <div style={{ ...styles.hint, marginTop: 8, color: "#E06C6C" }}>
                                В ростере всего {roster.length} игрок(ов), а для {MODE_LABEL[tour.mode]} нужно {need}. Добавьте
                                людей во вкладке «Команды».
                              </div>
                            ) : (
                              <>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                                  {roster.map((m) => {
                                    const on = chosen.includes(m.member_name);
                                    return (
                                      <button
                                        key={m.member_name}
                                        onClick={() => toggleLineupMember(tour.id, m.member_name, need)}
                                        style={{
                                          ...styles.memberChip,
                                          cursor: "pointer",
                                          background: on ? "#2A1A1C" : "transparent",
                                          borderColor: on ? "#D9414C" : "#3D2226",
                                          color: on ? "#F3ECEA" : "#8C7876",
                                        }}
                                      >
                                        {on ? "✓ " : ""}
                                        {m.member_name}
                                ...
                                {coach && (
                                  <div style={{ ...styles.hint, marginTop: 6 }}>
                                    Тренер <b style={{ color: "#E8A33D" }}>{coach.member_name}</b> игрового слота не занимает.
                                  </div>
                                )}
                                <button
                                  className="nur-btn"
                                  style={{ ...styles.accentBtnSm, marginTop: 10, opacity: enough ? 1 : 0.5 }}
                                  disabled={!enough}
                                  onClick={() => registerTeam(tour.id, pickedTeamId, chosen)}
                                >
                                  Зарегистрировать
                                </button>
                              </>
                        ...
                        <div style={{ width: "100%", marginTop: 10 }}>
                          {tour.status === "registration" && (
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                              <span style={styles.hint}>Жеребьёвка:</span>
                              <div style={styles.pillGroup}>
                                {[
                                  ...
                                  <button
                                    key={o.key}
                                    onClick={() => setTourSeeding(tour.id, o.key)}
                                    style={{ ...styles.pillBtn, ...((tour.seeding || "random") === o.key ? styles.pillBtnActive : {}) }}
                                  >
                                    <span style={styles.unskew}>{o.label}</span>
                                  </button>
                                ))}
                              </div>
                            </div>
                          )}
                          {tour.seeding === "manual" && registeredTeams.length >= 2 && (() => {
                            // Текущий порядок: черновик админа, иначе сохранённый
                            // seed, иначе порядок регистрации.
                            const order =
                              seedOrder[tour.id] ||
                              (tour.tournament_teams || [])
                                .slice()
                                .sort((a, b) => (a.seed ?? 9999) - (b.seed ?? 9999))
                                .map((tt) => tt.team_id);
                            // Порядок пишется в БД (tournament_teams.seed) сразу
                            // при каждой перестановке, а не только в состоянии
                            // браузера — иначе он молча терялся бы при обновлении
                            // страницы или если сетку формирует другой админ
                            ...
                            const move = async (idx, dir) => {
                              ...
                              setSeedOrder((prev) => ({ ...prev, [tour.id]: next }));
                              await Promise.all(
                                next.map((teamId, seedIdx) =>
                                  supabase.from("tournament_teams").update({ seed: seedIdx }).eq("tournament_id", tour.id).eq("team_id", teamId)
                                )
                              );
                              refreshTournaments();
                            };
                            return (
                              <div style={{ border: "1px solid #3D2226", padding: 12, marginBottom: 10 }}>
                                <div style={{ ...styles.cardTitle, fontSize: 13 }}>Ручная жеребьёвка</div>
                                <div style={{ ...styles.hint, marginBottom: 8 }}>
                                  Соседние пары играют друг с другом: 1-я со 2-й, 3-я с 4-й и так далее.
                                </div>
                                {order.map((tid, i) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        gap: 8,
                                        padding: "6px 8px",
                                        background: pairStart ? "#140F11" : "#0F0B0C",
                                        borderLeft: `3px solid ${pairStart ? "#D9414C" : "#3D2226"}`,
                                        marginBottom: 3,
                                      }}
                                    >
                                      <span style={{ ...styles.hint, width: 22 }}>{i + 1}</span>
                                      <span style={{ flex: 1, fontSize: 13 }}>{tm ? teamLabel(tm) : "—"}</span>
                                      <button style={styles.iconBtn} onClick={() => move(i, -1)} disabled={i === 0}>
                                        ↑
                                      </button>
                                      <button style={styles.iconBtn} onClick={() => move(i, 1)} disabled={i === order.length - 1}>
                                        ↓
                                      </button>
                                    </div>
                                  );
                ...
                {teams
                  ...
                  <div key={t.id} style={styles.card}>
                    <div style={styles.cardHeadRow}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={styles.badge}>
                          {(t.team_members || []).filter((m) => m.role !== "coach").length} игр.
                          {(t.team_members || []).some((m) => m.role === "coach") ? " · тренер" : ""}
                        </span>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                      {(t.team_members || [])
                        .slice()
                        .sort((a, b) => (a.role === "coach" ? 1 : 0) - (b.role === "coach" ? 1 : 0))
                        .map((m, i) => (
                          <span
                            key={i}
                            style={
                              m.role === "coach"
                                ? { ...styles.memberChip, borderColor: "#4A3A1E", color: "#E8A33D" }
                                : styles.memberChip
                            }
                          >
                            {m.member_name}
                    ...
                    {(t.team_members || []).length < (t.max_size || ROSTER_CAP) && (
                            ...
                            {addMemberResults[t.id].map((p) => (
                              <div key={p.id} style={{ ...styles.suggestItem, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button className="nur-btn" style={styles.accentBtnSm} onClick={() => inviteToTeam(t, p, "player")}>
                                    <Plus size={12} /> Игроком
                                  </button>
                                  <button
                                    className="nur-btn"
                                    style={{ ...styles.ghostBtnSm, borderColor: "#E8A33D", color: "#E8A33D" }}
                                    onClick={() => inviteToTeam(t, p, "coach")}
                                  >
                                    Тренером
                                  </button>
                                </div>
          ...
          <div style={styles.stack}>
            ...
            {showCreateLfg && session && (
              <div style={styles.card}>
                ...
                <div style={{ ...styles.hint, marginTop: 14, marginBottom: 6 }}>Telegram для связи (необязательно):</div>
                <input
                  className="nur-in"
                  value={newLfgTelegram}
                  onChange={(e) => setNewLfgTelegram(e.target.value)}
                  onKeyDown={playTypeSound}
                  placeholder="username (без @)"
                  style={{ ...styles.input, width: "100%" }}
                />
                <div style={styles.hint}>Покажется на карточке анкеты как быстрая ссылка, в дополнение к переписке на сайте.</div>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {p.telegram_username && (
                              <a
                                ...
                                onClick={(e) => e.stopPropagation()}
                              >
                                <LinkIcon size={12} /> @{p.telegram_username}
                              </a>
                            )}
                          </div>
                        </div>
            ...
            {session && (profile?.is_admin || profile?.is_moderator) && (
              <>
                ...
                <div className="nur-mode-card" style={{ ...styles.card, position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ marginTop: 12 }}>
                    <div style={{ ...styles.hint, marginBottom: 6 }}>Где хостится:</div>
                    <div style={styles.pillGroup}>
                      {[
                        { key: "cybershoke", label: "NUR / CyberShoke" },
                        { key: "faceit", label: "FACEIT" },
                      ].map((p) => (
                        <button
                          key={p.key}
                          onClick={() => setNewTourPlatform(p.key)}
                          style={{ ...styles.pillBtn, ...(newTourPlatform === p.key ? styles.pillBtnActive : {}) }}
                        >
                          <span style={styles.unskew}>{p.label}</span>
                        </button>
                      ))}
                    </div>
                    {newTourPlatform === "faceit" && (
                      <div style={{ marginTop: 8 }}>
                        <input
                          className="nur-in"
                          placeholder="Ссылка на турнир на FACEIT (faceit.com/.../tournament/...)"
                          value={newTourFaceitUrl}
                          onChange={(e) => setNewTourFaceitUrl(e.target.value)}
                          style={{ ...styles.input, width: "100%" }}
                        />
                        <div style={{ ...styles.hint, marginTop: 6 }}>
                          Сначала создай турнир на самом FACEIT, потом вставь сюда его ссылку. Сетка, вето и лобби ведутся на
                          FACEIT — здесь только показываются результаты (обновляются раз в несколько минут).
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ marginTop: 12 }}>
                    <div style={{ ...styles.hint, marginBottom: 6 }}>Жеребьёвка:</div>
                    <div style={styles.pillGroup}>
                      {[
                        { key: "random", label: "Случайная" },
                        { key: "manual", label: "Вручную" },
                        ...
                        <button
                          key={o.key}
                          onClick={() => setNewTourSeeding(o.key)}
                          style={{ ...styles.pillBtn, ...(newTourSeeding === o.key ? styles.pillBtnActive : {}) }}
                        >
                          <span style={styles.unskew}>{o.label}</span>
                        </button>
                      ))}
                    ...
                    <div style={{ ...styles.hint, marginTop: 6 }}>
                      {newTourSeeding === "random"
                        ? "Пары первого раунда система перемешает сама."
                        : "Перед формированием сетки вы сами расставите команды по порядку."}
                    </div>
                  </div>
...
const styles = {
  ...
  secNum: { fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700, color: "#8E2B33", letterSpacing: 0.5 },
  ...
  // ---------- Карточка турнира в стиле трансляции ----------
  tourCardOuter: { border: "1px solid #241417", background: "#0F0B0C", overflow: "hidden", marginBottom: 14 },
  tourHero: {
    position: "relative",
    overflow: "hidden",
    background: "#1A1013",
  ...
  tourGhost: {
    ...
    fontSize: 104,
    lineHeight: 1,
    color: "rgba(217,65,76,0.09)",
    pointerEvents: "none",
    userSelect: "none",
  },
  tourHeroMetaRow: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8 },
  tourStatusPill: {
    fontFamily: "'JetBrains Mono', monospace",
    fontSize: 9.5,
    fontWeight: 700,
  ...
  tourStatusPillLive: { background: "#D9414C", color: "#2C090C" },
  tourHeroMeta: { fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#8C7876", letterSpacing: 0.8, textTransform: "uppercase" },
  tourHeroTitle: {
    fontFamily: "'Anton', sans-serif",
    fontSize: 26,
    letterSpacing: 0.5,
  ...
  tourActionRow: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 4 },
  tourStartMeta: { fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#8C7876", letterSpacing: 0.5 },
  ...
  statsRow: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, marginTop: 1, background: "#241417", border: "1px solid #241417" },
  statCard: {
    border: "none",
    borderRadius: 0,
    padding: "16px 18px",
