...
export default function App() {
  ...
  const refreshFriends = useCallback(async (userId) => {
    ...
    const sentIds = rows.filter((r) => r.status === "pending" && r.sender_id === userId).map((r) => r.receiver_id);
    setFriends(accepted);
    setIncomingRequests(incoming);
    setSentPendingIds(sentIds);
  }, []);
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .nur-smoke {
          ...
          opacity: 0.9;
          background:
            radial-gradient(ellipse 40% 70% at 15% 30%, rgba(217,65,76,0.30), transparent 60%),
            radial-gradient(ellipse 35% 60% at 85% 60%, rgba(217,65,76,0.20), transparent 60%),
            radial-gradient(ellipse 50% 80% at 50% 100%, rgba(232,163,61,0.14), transparent 65%);
          filter: blur(26px);
            ...
            <div style={styles.modalBackdrop} onClick={() => setExpandedTour(null)}>
              <div style={styles.modalPanel} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
                  ...
                  <div style={styles.modalBody}>{renderBracket(modalTour.id, expandedRounds, false)}</div>
                </div>
              </div>
            </div>
          );
      ...
      {activeChatFriend && (
        <div style={styles.chatPanel}>
          <div style={styles.chatHeader}>
            ...
            <button style={styles.iconBtn} onClick={() => setActiveChatFriend(null)}>
