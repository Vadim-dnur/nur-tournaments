  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages" }, (payload) => {
        ...
        const currentTargetId = supportTargetRef.current ? supportTargetRef.current.id : session.user.id;
        if (msg.user_id === currentTargetId) {
          setSupportMessages((prev) => [...prev, msg]);
          if (!supportPanelOpenRef.current) setSupportUnread((prev) => prev + 1);
        } else {
    ...
    <div style={styles.page}>
      ...
      {session && !supportPanelOpen && (
        <button
          ...
          onClick={() => openSupportChat(profile?.is_admin || profile?.is_moderator ? supportTarget : null)}
        >
        ...
        <div style={styles.supportPanel}>
          <div style={styles.chatHeader}>
            <div style={{ flex: 1, minWidth: 0 }}>
              {supportTarget ? (
                <span style={{ color: "#F3ECEA", fontSize: 13.5, fontWeight: 600 }}>Тикет: {supportTarget.username}</span>
              ) : (
                <>
