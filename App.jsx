    ...
    <div style={styles.page}>
      ...
      {(() => {
        ...
        if (dismissedVetoIds.has(myVetoRaw.id)) {
          ...
          // даём маленькую плавающую кнопку, чтобы можно было вернуться,
          // если закрыл случайно (сама заявка на бан карт никуда не делась).
          return (
            <button
              className="nur-btn"
              style={{ position: "fixed", left: 20, bottom: 90, zIndex: 60, ...styles.ghostBtnSm, boxShadow: "0 6px 20px rgba(0,0,0,0.4)" }}
      ...
      {(() => {
        if (!currentUsername) return null;
        const myTeamIds = teams.filter((t) => (t.team_members || []).some((m) => m.member_name === currentUsername)).map((t) => t.id);
        const myLobby = matchLobbies.find(
          (l) => !dismissedLobbyIds.has(l.id) && l.status !== "finished" && (myTeamIds.includes(l.team1_id) || myTeamIds.includes(l.team2_id))
        );
