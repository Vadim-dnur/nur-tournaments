...
export default function App() {
  ...
  const [soundPresetId, setSoundPresetId] = useState(() => {
    try {
      return localStorage.getItem("nur-type-sound-preset") || "soft-click";
    } catch {
      return "soft-click";
    }
  });
  ...
  const SOUND_PRESETS = [
    { id: "soft-click", name: "Мягкий клик", type: "sine", freq: 720, dur: 0.06 },
    { id: "typewriter", name: "Печатная машинка", type: "square", freq: 260, dur: 0.035 },
    { id: "droplet", name: "Капля", type: "sine", freq: 900, freqEnd: 300, dur: 0.09 },
    { id: "glass", name: "Стекло", type: "triangle", freq: 1300, dur: 0.05 },
    { id: "thud", name: "Глухой стук", type: "sawtooth", freq: 150, dur: 0.08 },
    { id: "double", name: "Двойной клик", type: "sine", freq: 800, dur: 0.03, double: true },
  ];
  ...
  const playPresetSound = (presetId) => {
    ...
    try {
      ...
      const fire = (delay = 0) => {
        ...
        osc.frequency.setValueAtTime(preset.freq, ctx.currentTime + delay);
        if (preset.freqEnd) {
          osc.frequency.exponentialRampToValueAtTime(preset.freqEnd, ctx.currentTime + delay + preset.dur);
        }
        gain.gain.setValueAtTime(0.07, ctx.currentTime + delay);
        gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + delay + preset.dur);
        osc.connect(gain);
        gain.connect(ctx.destination);
        osc.start(ctx.currentTime + delay);
        osc.stop(ctx.currentTime + delay + preset.dur + 0.02);
      };
      fire(0);
      if (preset.double) fire(0.08);
    } catch {
      /* ignore audio errors */
    }
  };
  ...
  const saveAdSlot = async (ad, file) => {
    ...
    const expires_at = adExpiryDrafts[ad.slot] !== undefined ? adExpiryDrafts[ad.slot] || null : ad.expires_at || null;
                      ...
                      <div key={ad.id} style={{ border: "1px solid #3D2226", borderRadius: 8, padding: 12 }}>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 8, alignItems: "center", flexWrap: "wrap" }}>
                          <div>
                            ...
                            <input
                              type="datetime-local"
                              value={adExpiryDrafts[ad.slot] !== undefined ? adExpiryDrafts[ad.slot] : ad.expires_at ? ad.expires_at.slice(0, 16) : ""}
